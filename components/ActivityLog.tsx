    ...
    return (
      <div className="max-w-2xl mx-auto space-y-8">
        <header>
          ...
          <p className="text-slate-500">What is happening now?</p>
        </header>
          ...
          {types.map((type) => {
            ...
            return (
              ...
              >
                ...
                <span className={`font-bold transition-colors text-sm ${isSelected ? 'text-indigo-700' : 'text-slate-700'}`}>
                  {type.name}
                </span>
              </button>
            );
        ...
        {selectedTypeIds.length > 0 && (
          <div className="fixed bottom-24 left-1/2 -translate-x-1/2 w-full max-w-md px-4 md:left-auto md:right-8 md:translate-x-0">
            ...
            >
              <span>Add Specifics ({selectedTypeIds.length})</span>
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8 pb-12">
      <header className="flex items-center space-x-4">
        <button onClick={() => setIsConfiguring(false)} className="w-10 h-10 rounded-full flex items-center justify-center bg-white border border-slate-200 text-slate-500">
          <i className="fa-solid fa-arrow-left"></i>
        </button>
        <h2 className="text-2xl font-bold text-slate-800">Review Logs</h2>
      </header>

      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-indigo-600 p-6 rounded-3xl shadow-xl shadow-indigo-100">
          <label className="block text-xs font-bold text-indigo-100 uppercase mb-2 tracking-wider">Date & Time</label>
          <input
            type="datetime-local"
            value={sharedTimestamp}
            onChange={(e) => setSharedTimestamp(e.target.value)}
            className="w-full px-4 py-3 rounded-xl bg-white/10 border border-white/20 text-white focus:bg-white/20 outline-none"
            required
          />
        </div>
            ...
            return (
              <div key={id} className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {type.unit && (
                    <div>
                      <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1.5 ml-1">Amount ({type.unit})</label>
                      <input
                        type="number"
                        step="any"
                        placeholder="--"
                        ...
                        className="w-full px-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none"
                      />
                    </div>
                  )}
                  <div className={type.unit ? "" : "md:col-span-2"}>
                    <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1.5 ml-1">Note</label>
                    <input
                      type="text"
        ...
        <button type="submit" className="w-full bg-indigo-600 text-white font-bold py-4 rounded-2xl shadow-xl hover:bg-indigo-700 transition-all text-lg">
          Complete Logging
        </button>
      </form>
    </div>
